...
!*                                                                       *
!*                                                                       *
!*     by Napsu Karmitsa 2004 - 2016 (last modified 13.02.2019)          *
!*                                                                       *
!*                                                                       *
!*     This file is specially modified to solve clusterwise linear       *
!*     regression problems.                                              *
!*                                                                       *
!*************************************************************************
!*
...
MODULE lmbm_mod  ! Limited memory bundle method
    ...
    PRIVATE :: &
        lmbm, &            ! S Limited memory bundle subroutine for CLR
...
CONTAINS
    ...
    SUBROUTINE optim2(x0,x1,fvalue)
        ...
        USE initlmbm, ONLY : &
            n, &           ! Number of variables.
                           ! Jostain syystä tämä n nyt muuttuu kesken kaiken! tarkista initialization.
    ...
    SUBROUTINE lmbm(mc,f,nit,nfe,nge,iterm)
        ...
        iteration: DO
            ...
            !IF (xnorm <= tolg .AND. mcc == 0) THEN
            !   IF (gnorm < small) THEN
            !       PRINT*,'hätägradientti käyttöön!',iters
            !       do j=1,n
            !       x(j)=x(j)+0.0010
            !       CALL myf(n,x,ftmp,iterm)
            !       g(j)=(f-ftmp)/0.0010_prec
            !       x(j)=x(j)-0.0010
            !       end do
            !       PRINT*,DOT_PRODUCT(g,g)
            !       d=g
            ...
            !PRINT*,'hihu',nit,tmax,dnorm
            !IF (tmax <= tmin) tmax = 3*tmin
            IF (tmax < one) tmax = one
        ...
        !PRINT*,'hihu2',nit
            ...
            ELSE IF (inma == 2) THEN! Line search with directional derivatives which allows null steps
                 ! This is the only one with which the global convergence can be guaranteed.
                theta = one
...
!************************************************************************
    ...
    SUBROUTINE armijo(x,g,d,xo,fo,f,tin,p,alfn,wk,epsr,iters,nfe,nge,iterm)
        ...
        ! Local Scalars
        REAL(KIND=prec) :: &
            fu, &          ! Auxiliary values of the objective functions
...
!************************************************************************
    ...
    SUBROUTINE nmarmijo(x,g,d,xo,fo,f,fold,tin,p,alfn,wk,epsr,iters,nfe,nge,inewnma,iterm)
        ...
        ! Local Scalars
        REAL(KIND=prec) :: &
            fu, &          ! Auxiliary values of the objective function
            a_orig, &      ! Armijo parameter.
            inv_a_orig, &  ! Inverse of Armijo parameter.
            alpha, &       ! Armijo step size.
            dnorm, &       ! |d|
    ...
    IF (iprint >= 0) THEN
        ...
        IF (iterm == -5) THEN
            ...
            IF (nout == 3) WRITE (6,FMT='(1X,''Error: &
               &the size of the bundle (na) is too small, iterm='',I3)'            ) iterm
...
SUBROUTINE rprint(n,nit,nfe,nge,x,f,wk,qk,iterm,iprint)
    ...
    IF (iterm == 0) THEN
        ...
        IF (iprint == 5) WRITE (6,FMT='(1X,''x='',5D15.7:/(4X,5D15.7))'        )(x(i),i=1,n)
        RETURN
    END IF
